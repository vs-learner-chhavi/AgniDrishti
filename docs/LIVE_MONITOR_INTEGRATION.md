...
| Recent imagery | “Inspect satellite view” switches to an Esri satellite basemap and zooms to the detection. | A dated Sentinel/Landsat imagery service returning acquisition date, cloud cover and image/tile URL. | Analysts can inspect a time-relevant image rather than a visual basemap only. |
