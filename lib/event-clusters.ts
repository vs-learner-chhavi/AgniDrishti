...
export type EventCluster<T extends ClusterableEvent> = {
  id: string;
  events: T[];
...
export function buildEventClusters<T extends ClusterableEvent>(events: T[], radiusKm = 25, windowHours = 168): EventCluster<T>[] {
  const usable = events.filter((event, index, list) => {
    const lat = toCoordinate(event.lat);
    const lon = toCoordinate(event.lon);
  ...
  for (const seed of usable) {
    ...
    while (queue.length) {
      ...
      for (const candidate of usable) {
        if (!visited.has(candidate.id) && areEventsRelated(current, candidate, radiusKm, windowHours)) {
