...
export function buildEventClusters<T extends ClusterableEvent>(events: T[], radiusKm = 25, windowHours = 168): EventCluster<T>[] {
  const usable = events.filter((event, index, list) => { const lat = toCoordinate(event.lat); const lon = toCoordinate(event.lon); return lat !== null && lon !== null && Math.abs(lat) <= 90 && Math.abs(lon) <= 180 && list.findIndex(item => item.id === event.id) === index; });
  ...
  for (const seed of usable) {
    ...
    while (queue.length) { const current = queue.shift()!; members.push(current); for (const candidate of usable) if (!visited.has(candidate.id) && areEventsRelated(current, candidate, radiusKm, windowHours)) { visited.add(candidate.id); queue.push(candidate); } }
