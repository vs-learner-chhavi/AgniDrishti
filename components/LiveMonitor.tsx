...
function fromSignal(s: ThermalSignal): MonitorEvent {
  return { id: s.id, name: 'NASA FIRMS archived detection', lat: s.latitude, lon: s.longitude,
    cls: 'Unclassified thermal anomaly', confidence: 0, risk: s.frp >= 80 || s.brightness >= 355 ? 'CRITICAL' : s.frp >= 35 || s.brightness >= 340 ? 'HIGH' : 'MODERATE', brightness: s.brightness,
  ...
  return <section id="monitor" className="section sectionBlock restoredMonitor">
   ...
   <div className="monitorGrid">
    ...
    <aside className="selectedPanel">
     ...
     <div className="classification"><div><span>{selected.confidence?'LIKELY SOURCE':'CLASSIFICATION STATUS'}</span><b>{selected.confidence?selected.cls:'Awaiting model'}</b><small>{selected.confidence?isDemo?'Fixed demo classification':'Trained-model prediction':'Real FIRMS observation · not yet classified'}</small></div>{selected.confidence>0&&<strong>{selected.confidence}%<small>confidence</small></strong>}</div>
