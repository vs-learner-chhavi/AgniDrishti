  ...
  useEffect(() => {
    ...
    const draw = (attempt = 0) => {
      ...
      const addMarker = (e: MapEvent) => {
        ...
        tooltip.textContent = `${e.classification} · ${latitude.toFixed(3)}, ${longitude.toFixed(3)} · ${safeNumber(e.brightness).toFixed(1)} K`;
        marker.bindTooltip(
          tooltip,
      ...
      if (mode === 'density' && densityEvents.length && heatReady && (L as any).heatLayer) {
        try {
          ...
          // Selectable observations remain available if the heat canvas fails.
        }
      } else {
        events.forEach((event) => {
          const colour = colours[event.classification] || '#9bb4c6';
          if (layers.halos && ['HIGH', 'CRITICAL'].includes(event.risk)) {
      ...
      // Keep selected detections and fixed demos inspectable over density.
      if (mode === 'density') events.forEach(addMarker);
      ...
      if (layers.facilities) {
        facilities.forEach((facility) => {
          ...
          const tooltip = document.createElement('span');
          tooltip.textContent = `${facility.name} · ${facility.type} · ${safeNumber(facility.distanceKm).toFixed(1)} km`;
          marker.bindTooltip(
            tooltip,
            { direction: 'top' },
          );
