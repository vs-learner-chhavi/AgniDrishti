...
type Facility={name:string;type:string;distanceKm:number;lat:number;lon:number};
function bearingDeg(lat1:number,lon1:number,lat2:number,lon2:number){const toRad=(x:number)=>x*Math.PI/180,toDeg=(x:number)=>x*180/Math.PI;const dLon=toRad(lon2-lon1),y=Math.sin(dLon)*Math.cos(toRad(lat2)),x=Math.cos(toRad(lat1))*Math.sin(toRad(lat2))-Math.sin(toRad(lat1))*Math.cos(toRad(lat2))*Math.cos(dLon);return(toDeg(Math.atan2(y,x))+360)%360;}
function compassLabel(deg:number){const dirs=['N','NNE','NE','ENE','E','ESE','SE','SSE','S','SSW','SW','WSW','W','WNW','NW','NNW'];return dirs[Math.round(deg/22.5)%16];}
function slaThresholds(risk:string){if(risk==='CRITICAL')return{warn:5*60000,breach:10*60000};if(risk==='HIGH')return{warn:15*60000,breach:30*60000};return{warn:30*60000,breach:60*60000};}
function elapsedLabel(ms:number){const m=Math.floor(ms/60000);if(m<1)return'just now';if(m<60)return`${m}m ago`;const h=Math.floor(m/60);return`${h}h ${m%60}m ago`;}
...
export default function Home(){
 ...
 useEffect(()=>{setPickedFacility(null)},[selected.id]);
 ...
 return <main className="app">
  ...
  <section id="cross-event-intelligence" className="section sectionBlock">
    <SectionHeading
      kicker="02 · CROSS-EVENT INTELLIGENCE"
      title="See patterns across events."
      text="Group nearby thermal anomalies across space and time to surface emerging clusters, shared classifications, and escalation signals."
    />
    <CrossEventIntelligence
      events={events}
      selected={selected}
      onSelect={(event) => {
        const next = events.find((x) => x.id === event.id);
...
<line x1="100" y1="10" x2="100" y2="190" className="radarAxis"/><line x1="10" y1="100" x2="190" y2="100" className="radarAxis"/>
<g className="radarSweep"><path d="M100,100 L100,10 A90,90 0 0,1 163.6,36.4 Z"/></g>
{selected.facilities?.slice(0,10).map((f,i)=>{const brg=bearingDeg(selected.lat,selected.lon,f.lat,f.lon)*Math.PI/180;const r=Math.min(90,10+Math.min(f.distanceKm,10)/10*80);const x=100+r*Math.sin(brg),y=100-r*Math.cos(brg);const cls=f.distanceKm<=1?'dotNear':f.distanceKm<=5?'dotMid':'dotFar';const active=pickedFacility?.name===f.name&&pickedFacility?.distanceKm===f.distanceKm;return <circle key={i} cx={x} cy={y} r={active?7:f.distanceKm<=1?5:3.5} className={`radarDot ${cls}${active?' radarDotActive':''}`} onClick={()=>setPickedFacility(active?null:f)} style={{cursor:'pointer'}}><title>{f.name} · {f.distanceKm.toFixed(1)} km</title></circle>})}
<circle cx="100" cy="100" r="5" className="radarCenter"/>
</svg>
...
function contactFor(cls:string):{authority:string;phone:string;note:string}{const m:Record<string,{authority:string;phone:string;note:string}>={'Industrial Fire':{authority:'State Fire & Emergency Services',phone:'101',note:'Notify local fire control room and the facility operator if identifiable.'},'Gas Flare':{authority:'PESO / State Pollution Control Board',phone:'1800-11-0093',note:'Route to oil & gas safety authority; flares near residential zones need priority review.'},'Crop Burning':{authority:'State Pollution Control Board',phone:'1800-11-0093',note:'Typically handled by agricultural extension + pollution control, not emergency response.'},'Wildfire':{authority:'State Forest Department + Fire Services',phone:'101',note:'Escalate to forest fire control room; check wind direction before dispatch.'},'Mining':{authority:'Directorate General of Mines Safety',phone:'1800-11-0093',note:'Route to DGMS regional office and state mining department.'}};return m[cls]||{authority:'District Disaster Management Authority',phone:'112',note:'Default escalation path for unclassified or ambiguous thermal events.'};}
function ClassIcon({cls}:{cls:string}){const props={size:18};switch(cls){case 'Industrial Fire':return <Factory {...props}/>;case 'Gas Flare':return <Fuel {...props}/>;case 'Crop Burning':return <Sprout {...props}/>;case 'Wildfire':return <Flame {...props}/>;case 'Mining':return <Mountain {...props}/>;default:return <ShieldAlert {...props}/>;}}
